"""EMD Sifting Iteration 3 - Bandwidth h=0.05

Shows third sifting iteration.
Bandwidth: h3 = h1/2 = 0.05
"""
...
ax.plot(x, r3_noisy, color=MLGRAY, alpha=0.4, linewidth=0.8, label='Residual (noisy)')
ax.plot(x, r3_clean, color=MLBLUE, linewidth=1.5, alpha=0.7, label='Residual (clean)')
ax.plot(x, m3_noisy, color=MLRED, linewidth=2.5, label=f'IMF estimate (noisy, h={h3})')
ax.plot(x, m3_clean, color=MLGREEN, linewidth=2, linestyle='--', label=f'IMF estimate (clean, h={h3})')

ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_title(f'Sifting Iteration 3: h = {h3} (h1/2)')
