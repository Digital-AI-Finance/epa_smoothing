"""EMD Sifting Iteration 5 - Bandwidth h=0.025

Shows fifth sifting iteration (final in sequence).
Bandwidth: h5 = h1/4 = 0.025
"""
...
r5_noisy = r4_noisy - m4_noisy
r5_clean = r4_clean - m4_clean

# Iteration 5
m5_noisy = epa_local_median(x, r5_noisy, h5)
m5_clean = epa_local_median(x, r5_clean, h5)
...
ax.plot(x, r5_noisy, color=MLGRAY, alpha=0.4, linewidth=0.8, label='Residual (noisy)')
ax.plot(x, r5_clean, color=MLBLUE, linewidth=1.5, alpha=0.7, label='Residual (clean)')
ax.plot(x, m5_noisy, color=MLRED, linewidth=2.5, label=f'IMF estimate (noisy, h={h5})')
ax.plot(x, m5_clean, color=MLGREEN, linewidth=2, linestyle='--', label=f'IMF estimate (clean, h={h5})')

ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_title(f'Sifting Iteration 5: h = {h5} (h1/4)')
